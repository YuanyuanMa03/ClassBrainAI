#!/usr/bin/env python3
"""
ClassBrainAI · Skill 管理工具
支持从本地或远程 URL 添加、更新、查看和移除 skills

Usage:
  python3 scripts/skill_manager.py add-remote --agent study --name code_review \\
    --source https://raw.githubusercontent.com/org/skills/main/code_review/SKILL.md \\
    --description "代码审查"

  python3 scripts/skill_manager.py list-remote

  python3 scripts/skill_manager.py update-remote --agent study --name code_review

  python3 scripts/skill_manager.py remove-remote --agent study --name code_review

  python3 scripts/skill_manager.py import-official-hub --agents study,chair,vice_monitor
"""
import sys
import json
import pathlib
...
# 班委技能映射（ClassBrainAI 主题）
SKILL_AGENT_MAPPING = {
    'code_review': ('technical', 'discipline', 'chair'),
    'api_design': ('technical', 'logistics', 'chair'),
    'security_audit': ('discipline', 'chair'),
    'data_analysis': ('finance', 'chair'),
    'doc_generation': ('arts', 'chair'),
    'test_framework': ('logistics', 'discipline', 'chair'),
}
...
def import_official_hub(agent_ids: list) -> bool:
    ...
    for skill_name, url in OFFICIAL_SKILLS_HUB.items():
        ...
        if not agent_ids:
            target_agents = SKILL_AGENT_MAPPING.get(skill_name, ['chair'])
...
def main():
    parser = argparse.ArgumentParser(description='ClassBrainAI Skill 管理工具',
