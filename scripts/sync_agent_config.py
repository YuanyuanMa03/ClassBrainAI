...
# ClassBrainAI 班委角色映射
ID_LABEL = {
    'monitor':        {'label': '班长',    'role': '班长',     'duty': '消息分拣与任务分派',      'emoji': '👨‍🎓'},
    'study':          {'label': '学习委员', 'role': '学习委员', 'duty': '学习规划与任务分解',      'emoji': '📚'},
    'chair':          {'label': '班委主席', 'role': '班委主席', 'duty': '审议与质量把关',         'emoji': '🪑'},
    'vice_monitor':   {'label': '副班长',   'role': '副班长',   'duty': '任务派发与协调汇总',      'emoji': '👨‍💼'},
    'technical':      {'label': '技术委员', 'role': '技术委员', 'duty': '代码开发与技术支持',      'emoji': '💻'},
    'logistics':      {'label': '后勤委员', 'role': '后勤委员', 'duty': '基础设施与工具维护',      'emoji': '🔧'},
    'finance':        {'label': '财务委员', 'role': '财务委员', 'duty': '数据处理与资源管理',      'emoji': '💰'},
    'arts':           {'label': '文艺委员', 'role': '文艺委员', 'duty': '文档编写与知识管理',      'emoji': '🎨'},
    'discipline':     {'label': '纪律委员', 'role': '纪律委员', 'duty': '质量检查与规范维护',      'emoji': '📋'},
    'organization':   {'label': '组织委员', 'role': '组织委员', 'duty': '活动组织与人员协调',      'emoji': '🤝'},
    'morning_reading': {'label': '早读委员', 'role': '早读委员', 'duty': '每日信息采集与简报',      'emoji': '🌅'},
}
...
def main():
    ...
    # 补充不在 openclaw.json agents list 中的 agent（兼容旧版）
    EXTRA_AGENTS = {
        'monitor':        {'model': default_model, 'workspace': str(pathlib.Path.home() / '.openclaw/workspace-monitor'),
                          'allowAgents': ['study']},
        'study':          {'model': default_model, 'workspace': str(pathlib.Path.home() / '.openclaw/workspace-study'),
                          'allowAgents': ['chair', 'vice_monitor']},
        'chair':          {'model': default_model, 'workspace': str(pathlib.Path.home() / '.openclaw/workspace-chair'),
                          'allowAgents': ['study', 'vice_monitor']},
        'vice_monitor':   {'model': default_model, 'workspace': str(pathlib.Path.home() / '.openclaw/workspace-vice_monitor'),
                          'allowAgents': ['technical', 'logistics', 'finance', 'arts', 'discipline', 'organization']},
        'technical':      {'model': default_model, 'workspace': str(pathlib.Path.home() / '.openclaw/workspace-technical'),
                          'allowAgents': ['vice_monitor']},
        'logistics':      {'model': default_model, 'workspace': str(pathlib.Path.home() / '.openclaw/workspace-logistics'),
                          'allowAgents': ['vice_monitor']},
        'finance':        {'model': default_model, 'workspace': str(pathlib.Path.home() / '.openclaw/workspace-finance'),
                          'allowAgents': ['vice_monitor']},
        'arts':           {'model': default_model, 'workspace': str(pathlib.Path.home() / '.openclaw/workspace-arts'),
                          'allowAgents': ['vice_monitor']},
        'discipline':     {'model': default_model, 'workspace': str(pathlib.Path.home() / '.openclaw/workspace-discipline'),
                          'allowAgents': ['vice_monitor']},
        'organization':   {'model': default_model, 'workspace': str(pathlib.Path.home() / '.openclaw/workspace-organization'),
                          'allowAgents': ['vice_monitor']},
        'morning_reading': {'model': default_model, 'workspace': str(pathlib.Path.home() / '.openclaw/workspace-morning_reading'),
                          'allowAgents': []},
    }
    ...
    deploy_soul_files()
    # 同步 scripts/ 到各 workspace（保持 class_board.py 等最新）
...
def sync_scripts_to_workspaces():
    """将项目 scripts/ 目录同步到各 agent workspace（保持 class_board.py 等最新）"""
