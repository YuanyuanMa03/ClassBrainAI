...
def detect_official(agent_id):
    ...
    mapping = {
        ...
        'study':          ('学习委员', '学习委员'),
        'chair':          ('班委主席', '班委主席'),
        'vice_monitor':   ('副班长', '副班长'),
        'technical':      ('技术委员', '技术委员'),
        'logistics':      ('后勤委员', '后勤委员'),
        'finance':        ('财务委员', '财务委员'),
        'arts':           ('文艺委员', '文艺委员'),
        'discipline':     ('纪律委员', '纪律委员'),
        'organization':   ('组织委员', '组织委员'),
        'morning_reading': ('早读委员', '早读委员'),
    }
    return mapping.get(agent_id, ('副班长', '副班长'))
    ...
    try:
        ...
        if existing_tasks_file.exists():
            try:
                ...
                cb_existing = [t for t in existing if str(t.get('id', '')).startswith('CB')]

                tasks = [t for t in tasks if not str(t.get('id', '')).startswith('CB')]
                tasks = cb_existing + tasks
            except Exception as e:
                log.error(f'merge existing CB tasks failed: {e}')
                pass
