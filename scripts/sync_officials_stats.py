#!/usr/bin/env python3
"""同步各班委统计数据 → data/officials_stats.json"""
...
# ClassBrainAI 班委成员列表
OFFICIALS = [
    {'id':'monitor',        'label':'班长',     'role':'班长',       'emoji':'👨‍🎓','rank':'班级负责人'},
    {'id':'study',          'label':'学习委员', 'role':'学习委员',   'emoji':'📚','rank':'核心班委'},
    {'id':'chair',          'label':'班委主席', 'role':'班委主席',   'emoji':'🪑','rank':'核心班委'},
    {'id':'vice_monitor',   'label':'副班长',   'role':'副班长',     'emoji':'👨‍💼','rank':'核心班委'},
    {'id':'technical',      'label':'技术委员', 'role':'技术委员',   'emoji':'💻','rank':'班委成员'},
    {'id':'logistics',      'label':'后勤委员', 'role':'后勤委员',   'emoji':'🔧','rank':'班委成员'},
    {'id':'finance',        'label':'财务委员', 'role':'财务委员',   'emoji':'💰','rank':'班委成员'},
    {'id':'arts',           'label':'文艺委员', 'role':'文艺委员',   'emoji':'🎨','rank':'班委成员'},
    {'id':'discipline',     'label':'纪律委员', 'role':'纪律委员',   'emoji':'📋','rank':'班委成员'},
    {'id':'organization',   'label':'组织委员', 'role':'组织委员',   'emoji':'🤝','rank':'班委成员'},
    {'id':'morning_reading','label':'早读委员', 'role':'早读委员',   'emoji':'🌅','rank':'班委成员'},
]
...
def get_task_stats(org_label, tasks):
    ...
    participated = []
    for t in tasks:
        if not t['id'].startswith('CB'): continue
